...
test('turn from (1.842,-81.790) -1.830 to (1,1) is ~0.249', () => {
	const shipPos = { x: 1.842, y: -81.790 }
	const rotation = -1.830
	const targetPos = { x: 1, y: 1 }
	expect(turnTo(shipPos, rotation, targetPos)).toBeCloseTo(0.249, 3)
})

test('turn from (2,0) -pi/4 to (1,1) is -pi/2', () => {
	const shipPos = { x: 2, y: 0 }
	const rotation = -Math.PI/4
	const targetPos = { x: 1, y: 1 }
	expect(turnTo(shipPos, rotation, targetPos)).toBeCloseTo(-Math.PI/2, 4)
})