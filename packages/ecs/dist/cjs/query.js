...
function matches(type, filters, archetype) {
    return (type_1.typeIsSuperset(archetype.signature, type) &&
...
function createQueryInternal(options) {
    ...
    const signature = options.select
        .map(schema => component_1.registerSchema(schema))
        .sort((a, b) => a - b);
    const layout = (options.include ?? options.select).map((schema) => component_1.registerSchema(schema));
    const recordsIndex = [];
    ...
    function forEach(iteratee) {
        const c = context ?? internal_1.UNSAFE_internals.currentWorldId;
        core_1.assert(c !== null && c !== -1, ERROR_MSG_UNBOUND_QUERY, core_1.ErrorType.Query);
