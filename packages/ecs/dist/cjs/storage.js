...
function createStorage(options = {}) {
    const archetypes = options.snapshot
        ? options.snapshot.archetypes.map(snapshot => archetype_1.createArchetype({ snapshot }))
        : [archetype_1.createArchetype({ signature: [] })];
    const entityIndex = [];
    const entityRelocating = signal_1.createSignal();
    ...
    function getEntityArchetype(entity) {
        ...
        core_1.assert(archetype !== undefined, ERROR_ENTITY_NOT_CREATED);
        core_1.assert(archetype !== null, ERROR_ALREADY_DESTROYED);
        return archetype;
    }
    ...
    function attachOrUpdateComponents(entity, components) {
        ...
        core_1.mutableEmpty(tmpComponentsToInsert);
    ...
    function hasComponentOfSchema(entity, schema) {
        ...
        core_1.assert(type !== undefined, ERROR_NO_SCHEMA);
    ...
    function clear() {
        core_1.mutableEmpty(archetypes);
        core_1.mutableEmpty(entityIndex);
    }
    function getSnapshot() {
        return {
            archetypes: archetypes.map(archetype => ({
                ...
                indices: core_1.packSparseArray(archetype.indices),
            })),
        };
    }
    return {
